...
// ============================================================
// BYBIT PRICE FETCH — Reliable on Vercel
// ============================================================
const fetchKrakenPrices = async () => {
  try {
    const symbols = WATCHLIST.map(w => w.bybitSymbol);
    const prices = {};
    ...
    for (const coin of WATCHLIST) {
      const ticker = tickers.find(t => t.symbol === coin.bybitSymbol);
      const price = ticker?.lastPrice ? parseFloat(ticker.lastPrice) : null;
      prices[coin.key] = { price, display: fmt(price) };
    }

    console.log("Bybit prices OK — BTC:", prices.BTC?.display);
    return prices;
  } catch (e) {
    console.log("Bybit price error:", e.message);
    return null;
  }
};
