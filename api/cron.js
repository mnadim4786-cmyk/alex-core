...
// ============================================================
const supabaseFetch = async (table, options = {}, retries = 2) => {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      console.log("Supabase retry", i + 1, ":", e.message);
...
// ============================================================
// CRYPTO PRICES — Multi-source fallback (no regional blocks)
// Source 1: CryptoCompare (primary)
// Source 2: Coinpaprika (fallback)
// ============================================================
const getCryptoPrices = async () => {
  // Try CryptoCompare first
  try {
    const res = await fetchWT(
      'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BTC,ETH,SOL,ADA,AVAX,NEAR&tsyms=USD',
    ...
    const raw = d.RAW;
    const fmt = (coin) => {
      const price = raw?.[coin]?.USD?.PRICE;
      const chg = raw?.[coin]?.USD?.CHANGEPCT24HOUR;
      if (!price) return 'N/A';
      const priceStr = '$' + parseFloat(price).toLocaleString('en-US', { minimumFractionDigits: 2 });
      const chgStr = chg ? (chg > 0 ? ' 📈 +' + parseFloat(chg).toFixed(2) + '%' : ' 📉 ' + parseFloat(chg).toFixed(2) + '%') : '';
      return priceStr + chgStr;
    };
    console.log("Prices fetched via CryptoCompare");
    return {
      BTC:  fmt('BTC'),
      ETH:  fmt('ETH'),
      SOL:  fmt('SOL'),
      ...
      NEAR: fmt('NEAR'),
      raw: {
        bitcoin:        { usd: raw?.BTC?.USD?.PRICE,  usd_24h_change: raw?.BTC?.USD?.CHANGEPCT24HOUR  },
        ethereum:       { usd: raw?.ETH?.USD?.PRICE,  usd_24h_change: raw?.ETH?.USD?.CHANGEPCT24HOUR  },
        solana:         { usd: raw?.SOL?.USD?.PRICE,  usd_24h_change: raw?.SOL?.USD?.CHANGEPCT24HOUR  },
        cardano:        { usd: raw?.ADA?.USD?.PRICE,  usd_24h_change: raw?.ADA?.USD?.CHANGEPCT24HOUR  },
        'avalanche-2':  { usd: raw?.AVAX?.USD?.PRICE, usd_24h_change: raw?.AVAX?.USD?.CHANGEPCT24HOUR },
        'near-protocol':{ usd: raw?.NEAR?.USD?.PRICE, usd_24h_change: raw?.NEAR?.USD?.CHANGEPCT24HOUR }
      }
    };
  } catch (e) {
    console.log("CryptoCompare error:", e.message, "— trying Coinpaprika fallback");
  }

  // Fallback: Coinpaprika (different CDN, rarely blocked)
  try {
    const coins = [
      { id: 'btc-bitcoin',   key: 'BTC',  rawKey: 'bitcoin'         },
      { id: 'eth-ethereum',  key: 'ETH',  rawKey: 'ethereum'        },
      { id: 'sol-solana',    key: 'SOL',  rawKey: 'solana'          },
      { id: 'ada-cardano',   key: 'ADA',  rawKey: 'cardano'         },
      { id: 'avax-avalanche',key: 'AVAX', rawKey: 'avalanche-2'     },
      { id: 'near-near',     key: 'NEAR', rawKey: 'near-protocol'   }
    ];
    const results = { raw: {} };
    for (const coin of coins) {
      try {
        ...
        const chg   = data?.quotes?.USD?.percent_change_24h;
        const priceStr = price ? '$' + parseFloat(price).toLocaleString('en-US', { minimumFractionDigits: 2 }) : 'N/A';
        const chgStr   = chg ? (chg > 0 ? ' 📈 +' + parseFloat(chg).toFixed(2) + '%' : ' 📉 ' + parseFloat(chg).toFixed(2) + '%') : '';
        results[coin.key] = priceStr + chgStr;
        results.raw[coin.rawKey] = { usd: price, usd_24h_change: chg };
      } catch (err) {
        results[coin.key] = 'Offline';
        results.raw[coin.rawKey] = { usd: null, usd_24h_change: null };
      }
    }
    console.log("Prices fetched via Coinpaprika fallback");
    return results;
  } catch (e) {
    console.log("All price sources failed:", e.message);
