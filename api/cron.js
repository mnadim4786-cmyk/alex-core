...
const fetchKrakenPrices = async () => {
  try {
    ...
    for (const coin of WATCHLIST) {
      // Try primary pair name first, then alternatives
      const pair = r[coin.krakenPair] ||
                   r[coin.krakenPair.replace('XX', 'X').replace('ZUSD', 'USD')] ||
                   Object.values(r).find((_, i) => Object.keys(r)[i].includes(coin.key)) ||
                   null;
      const price = pair?.c?.[0] ? parseFloat(pair.c[0]) : null;
      prices[coin.key] = { price, display: fmt(price) };
    }
    console.log("Kraken prices OK — BTC:", prices.BTC?.display);
    return prices;
  } catch (e) {
    console.log("Kraken price error:", e.message);
    return null;
  }
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Always fetch live prices (Kraken — fast, reliable)
