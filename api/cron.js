...
// ============================================================
// PRICES FROM SUPABASE CACHE
// (Populated by api/prices.js every 5 minutes)
// ============================================================
const fetchKrakenPrices = async () => {
  try {
    const rows = await supabaseFetch('alex_config', {
      params: '?key=eq.cached_prices'
    });
    ...
    console.log("Prices from Supabase cache ✅ — BTC:", prices?.BTC?.display, "| Updated:", cached.updated_at);
    return prices;
  } catch (e) {
