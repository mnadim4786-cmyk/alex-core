...
# Target explicit mapping to match your Vercel logs exactly
...
def alex_handler(path=None):
    ...
    if not TELEGRAM_TOKEN:
        logging.critical("❌ TELEGRAM_TOKEN is missing in Vercel Env Vars!")
        return jsonify({"status": "missing_token"}), 200

    try:
        # Strict Diagnostic Response Framework
        diagnostic_echo = f"⚡ *Alex System Restructured Successfully!*\n\nSir, direct explicit path mapping `/api/webhook` activate ho chuki hai.\nReceived command: `{clean_markdown(user_query)}`"
