...
const supabaseSave = async (key, value) => {
  ...
  const patchRes = await fetchWT(
    SUPABASE_URL + '/rest/v1/alex_config?key=eq.' + encodeURIComponent(key),
    {
      method: 'PATCH',
      headers,
  ...
  // If PATCH updated 0 rows, INSERT new
  if (patchRes.ok) {
