...
const supabaseSave = async (key, value) => {
  const baseUrl = SUPABASE_URL + '/rest/v1/alex_cache';
  // Use service key for write operations
  const writeKey = SUPABASE_SERVICE_KEY || SUPABASE_KEY;
  const headers = {
    'apikey': writeKey,
  ...
  // Check if key exists
  const getRes = await fetchWT(baseUrl + '?key=eq.' + key, { method: 'GET', headers }, 8000);
  const existing = getRes.ok ? await getRes.json() : [];

  if (existing.length > 0) {
    // UPDATE
    const updateRes = await fetchWT(baseUrl + '?key=eq.' + key, {
      method: 'PATCH',
      headers,
    ...
    if (!updateRes.ok) {
      ...
      throw new Error("Supabase save error: " + updateRes.status + " " + err);
    }
    console.log("Supabase UPDATE success for key:", key);
    return updateRes.json();
  } else {
    // INSERT
    const insertRes = await fetchWT(baseUrl, {
      method: 'POST',
      headers,
    ...
    if (!insertRes.ok) {
      ...
      throw new Error("Supabase save error: " + insertRes.status + " " + err);
    }
    console.log("Supabase INSERT success for key:", key);
